import * as React from 'react';
export default function svgComp() {
  return (
    <svg
      id="z12391"
      className="acjk"
      version="1.1"
      viewBox="0 0 1024 1024"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink">
      <path
        id="z12391d1"
        d="M756 172C740 173 730 187 718 196C700 206 681 211 662 219C533 264 404 308 274 350C250 358 226 367 201 372C183 376 163 375 146 367C138 364 126 370 127 380C129 392 139 399 146 408C164 427 187 444 213 447C234 447 251 434 269 426C333 394 400 367 466 340C530 314 594 289 658 265C636 289 613 312 592 336C556 375 521 416 498 463C469 520 455 584 464 647C471 695 490 743 524 778C550 806 581 829 612 851C645 873 681 891 719 903C734 906 752 908 765 898C776 887 774 870 767 858C762 844 757 827 742 820C715 808 684 808 658 795C612 775 573 742 543 703C527 682 518 657 514 632C507 592 512 550 529 513C547 467 574 425 608 390C635 360 665 332 696 306C727 282 763 265 801 259C812 257 824 250 824 238C824 223 811 214 802 203C790 192 778 180 763 173C761 173 759 172 756 172Z"
      />
      <path
        id="z12391d2"
        d="M799 110C790 111 787 126 795 130C809 141 824 150 836 163C857 185 871 213 889 237C896 245 911 249 919 240C930 228 927 211 925 197C922 175 910 153 890 142C867 128 841 120 815 112C810 111 805 111 799 110Z"
      />
      <path
        id="z12391d3"
        d="M867 42C858 41 855 57 863 61C878 72 894 81 906 96C926 118 941 145 958 170C966 179 982 180 989 169C1000 155 995 136 992 120C987 101 976 82 957 72C933 58 905 49 878 42C874 42 871 41 867 42Z"
      />
      <defs>
        <clipPath id="z12391c1">
          <use xlinkHref="#z12391d1" />
        </clipPath>
        <clipPath id="z12391c2">
          <use xlinkHref="#z12391d2" />
        </clipPath>
        <clipPath id="z12391c3">
          <use xlinkHref="#z12391d3" />
        </clipPath>
      </defs>
      <path
        style={{'--d': '1s'} as any}
        pathLength="3333"
        clipPath="url(#z12391c1)"
        d="M 142,378 222,420 756,207 786,232 678,285 518,461 482,601 531,754 759,888"
      />
      <path
        style={{'--d': '2s'} as any}
        pathLength="3333"
        clipPath="url(#z12391c2)"
        d="M 799,124 883,178 899,242"
      />
      <path
        style={{'--d': '3s'} as any}
        pathLength="3333"
        clipPath="url(#z12391c3)"
        d="M 868,54 947,110 969,172"
      />
    </svg>
  );
}
